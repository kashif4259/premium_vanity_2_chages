...
export default {
    ...
    created() {
        this.deliveryOrPickup = this.edit_order_details.delivery_or_pickup;
        this.deliveryOrPickupDate = this.edit_order_details.delivery_or_pickup_date;
        this.deliveryCharges = this.edit_order_details.delivery_charges;
        this.salesNote = this.edit_order_details.sales_note;
    ...
    methods: {
        ...
        storeInvoiceCart(action) {
            ...
            if ((parseInt(this.balance) === 0 || this.paidAmount == this.finalCart.grandTotal) || (parseInt(this.paidAmount) === 0 && this.salesOrReceivingType === "internal-transfer")) {
                ...
                cartItemsToStore.salesProductVariations = this.salesProductVariations;
        ...
        onlineDataProcessing(cartItemsToStore, postURL, action) {
            ...
            console.log(cartItemsToStore);
            console.log("action: "+ action);
            console.log(this.edit_order_details);
            cartItemsToStore.oldOrderId = this.edit_order_details.id;
            cartItemsToStore.oldInvoiceId = this.edit_order_details.invoice_id;
            let instance = this;
            this.lastInvoiceNumber = this.lastInvoiceNumber + 1;
            cartItemsToStore.highest_invoice_number = this.lastInvoiceNumber;
            this.status.isSubmitted = true;
            let finalUrl = window.appConfig.publicPath+'/orders/'+this.edit_order_details.id+'/update';
            instance.axiosGETorPOST(
                {
                    url: '/orders/'+this.edit_order_details.id+'/update',
                ...
                (success, responseData) => {
                    if (success) {
                        ...
                        if (action === 'continue') {
                            ...
                            if(isredirect)
                            {
                                localStorage.setItem('getPackingSlipDetails', responseData.orderID);
    
                                window.location = window.appConfig.publicPath+'/orders/';
                            }
                        }
                    }
                    instance.status.hidePaymentListGetLoader = true;
